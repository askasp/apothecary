defmodule Apothecary.BrewSchedulerTest do
  ...
  alias Apothecary.{BrewScheduler, Worktrees}
  ...
  describe "status/0" do
    ...
    test "includes newly created enabled recipe" do
      {:ok, recipe} = Worktrees.create_recipe(%{title: "Test schedule", schedule: "0 0 * * *"})
  ...
  describe "recipe PubSub reactions" do
    test "schedules timer when recipe is created" do
      {:ok, recipe} = Worktrees.create_recipe(%{title: "Auto-schedule", schedule: "0 0 * * *"})
    ...
    test "removes timer when recipe is deleted" do
      {:ok, recipe} = Worktrees.create_recipe(%{title: "Delete me", schedule: "0 0 * * *"})
      _ = :sys.get_state(BrewScheduler)
    ...
    test "removes timer when recipe is disabled" do
      {:ok, recipe} = Worktrees.create_recipe(%{title: "Disable me", schedule: "0 0 * * *"})
      _ = :sys.get_state(BrewScheduler)
      ...
      # Toggle disabled
      Worktrees.toggle_recipe(recipe.id)
      _ = :sys.get_state(BrewScheduler)
    ...
    test "re-schedules timer when recipe is re-enabled" do
      {:ok, recipe} = Worktrees.create_recipe(%{title: "Re-enable", schedule: "0 0 * * *"})
      _ = :sys.get_state(BrewScheduler)
