defmodule Apothecary.Brewer do
  ...
  defp finalize_concoction(worktree_id, agent) do
    ...
    case {mode, auto} do
      {:github, true} -> finalize_github(worktree_id, agent, project_dir)
      {:github, false} -> finalize_github_manual(worktree_id, agent, project_dir)
      {:local, true} -> finalize_local_auto(worktree_id, agent, project_dir)
      {:local, false} -> finalize_local_manual(worktree_id)
    end
  end

  defp finalize_github(worktree_id, agent, project_dir) do
  ...
  defp finalize_local_auto(worktree_id, agent, project_dir) do
    # Auto-merge into main locally
    worktree_path = agent.worktree_path
    Logger.info("Auto-merging concoction #{worktree_id} into main (local+auto)")

    case Apothecary.Git.local_merge(project_dir, worktree_path) do
      :ok ->
  ...
  defp finalize_github_manual(worktree_id, agent, project_dir) do
